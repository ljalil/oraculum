...
class CMAPSSDataset():
    ...
    def RUL_modeler(self, life : int, kind : str, classes = 5):
        ...
        if kind == 'linear':
            ...
            x = np.concatenate(( self.constant_rul*np.ones((self.start_of_failure,1)), linspace(self.constant_rul,0,life-self.start_of_failure).reshape(-1,1)), axis=0)
            return x
    ...
    def construct_binary_classification_data(self, good_faulty_threshould = 30, batch_size = 64):
        x = np.zeros((0,26))
        y = np.zeros((0,1))
        ...
        for group_name, group_df in self.data.groupby('unit'):
            ...
            y = np.concatenate((y, np.zeros((good_faulty_threshould,1)), np.ones((good_faulty_threshould,1))), axis=0)
